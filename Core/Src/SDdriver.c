...
void OpenSD(void)
{
	...
	SDCardRes = f_mount(&fs, "0:", 1); // 挂载

	//	if(test_sd == 0)		//用于测试格式�?
	if (SDCardRes == FR_NO_FILESYSTEM) // 没有文件系统，格式化
	{
		//		test_sd =1;				//用于测试格式�?
		myPrintf("没有文件系统! \r\n");
		SDCardRes = f_mkfs("", 0, 0); // 格式化sd�?
		if (SDCardRes == FR_OK)
		{
			myPrintf("格式化成功! \r\n");
			SDCardRes = f_mount(NULL, "0:", 1); // 格式化后先取消挂�?
			SDCardRes = f_mount(&fs, "0:", 1);  // 重新挂载
			if (SDCardRes == FR_OK)
			{
				myPrintf("SD卡已经成功挂载，可以进进行文件写入测试!\r\n");
			}
		}
		else
		{
...
void WritetoSD(BYTE write_buff[], uint8_t bufSize, const char *FileName)
{
	FATFS fs;
	FIL file;
	uint8_t res = 0;
	...
	if (f_size(&file) == 0) // 如果文件为空，那么写入文件头
	{
		f_lseek(&file, f_size(&file)); // 确保写词写入不会覆盖之前的数据
	}
	else
	{
		f_lseek(&file, f_size(&file) - 1); // 确保写词写入不会覆盖之前的数据
	}
	// f_lseek(&file, f_size(&file)-1); // 确保写词写入不会覆盖之前的数�?
	if (res == FR_OK)
	{
		myPrintf("打开成功/创建文件成功 \r\n");
		res = f_write(&file, write_buff, bufSize, &Bw); // 写数据到SD�?
		if (res == FR_OK)
		{
			myPrintf("文件写入成功 \r\n");
		}
...
void Get_SDCard_Capacity(void)
{
	...
	if (res != FR_OK)
	{
		myPrintf("FileSystem Mounted Failed (%d)\r\n", result);
	}

	res = f_getfree("0:", &fre_clust, &_fs); /* 根目�? */
	if (res == FR_OK)
	{
		TotalSpace = (uint16_t)(((_fs->n_fatent - 2) * _fs->csize) / 2 / 1024);
		AvailableSize = (uint16_t)((fre_clust * _fs->csize) / 2 / 1024);
	...
	{
		myPrintf("Get SDCard Capacity Failed (%d)\r\n", result);
	}
}
